...
\begin{example}
    ...
    \]
    This function has form consistent with the bound in \cref{proposition:semigroup_meanzero_observable},
...
\begin{proposition}
    ...
    \begin{itemize}
        ...
        If also $\mathcal L \psi_{\vect e} \in L^2(\mu)$ and $\partial_q \mathcal \psi_{\vect e} \in L^2(\mu)$,
...
since the operator $\mathcal L^*$, which is the formal $L^2(\mu)$ adjoint of $\mathcal L$,
...
Here we employed the fact that, for any $f \in L^2(\mu)$ such that $\grad_q f \in L^2(\mu)$,
it holds
\[
    \ip{\grad_q (\id - \Pi_p) f}{\grad_q \Pi_p f} = 0,
\]
and so $\norm{(\id - \Pi_p) f}[1_q] \leq \norm{\grad_q f}[1_q]$.
The bound~\eqref{eq:refined_bound} is then obtained using the same reasoning as that which led to~\eqref{eq:bias}.
\end{proof}
