...
The Green--Kubo formula can also be derived from~\eqref{eq:effective_diffusion_poisson} with the representation formula $\phi_{\vect e} = \int_{0}^{\infty} \e^{\theta \mathcal L} \left(\vect e^\t \vect p\right) \, \d \theta$,
...
and the subspace of functions with average $0$ in $\vect p$ (with respect to the marginal distribution $\kappa$, defined in Eqn.~\eqref{eq:definition_prob_measures}, and for almost every $\vect q \in \torus^d$).
...
\begin{proof}
    ...
    \begin{align}
        \notag
        \var \bigl[v(T)\bigr]
        &= \min_{\mathscr V \in \real} \expect \Bigl[\left(v(T) - \mathscr V\right)^2\Bigr]
...
\begin{proposition}
    [Asymptotic value of the variance]
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        \hline
        % \# realizations for the calculation of $d[\psi]$ & $J$ & $10^7$ \\
        % \hline
    \end{tabular}
...
which enabled us to show formally that $D^{\gamma, \nu}$ behaves as $1/\gamma$ in the limit as $\gamma \to 0$,
with diffusion coefficient $D^{\rm und}_{\nu}$ that can be calculated efficiently and is different from $D^{\rm und}$.
We recall from the introduction that the diffusion coefficient $D^{\rm und}$ is defined as~$D^{\rm und} = \lim_{\gamma \to 0} \gamma D^{\gamma}$,
where $D^{\gamma}$ is the effective diffusion coefficient of the one-dimensional overdamped Langevin dynamics~\eqref{eq:effective_diffusion_poisson}.
...
% denoting by $\psi$ an approximation of $\phi_{\rm 1D}$,
% and assuming $\psi_{\vect e}(\vect q, \vect p) = \phi_0(q_1, p_1)$,
Specifically, we have
% assuming that $\psi_{\vect e}(\vect q, \vect p) = \phi_0(q_1, p_1)$, we have
\begin{equation}
...
% The
% ~\cite{MR3847747,MR4338293,pmlr-v145-zhai22a}.
...
The resolvent operator $\mathcal L_{\rm FD}^{-1} \colon L^2_0(\kappa) \to L^2_0(\kappa)$ is compact and self-adjoint.
Therefore, by the spectral theorem,
there exists an orthonormal basis of $L^2(\kappa)$ consisting of eigenfunctions of~$\mathcal L_{\rm FD}^{-1}$.
% The eigenfunctions of $\mathcal L_{\rm FD}$ are rescaled Hermite polynomials,
These, which are also eigenfunctions of $\mathcal L_{\rm FD}$, are given by rescaled Hermite polynomials.
The eigenvalue of~$\mathcal L_{\rm FD}$ associated with the constant polynomial is~0,
and all the other eigenvalues of this operator are integer numbers less than or equal to $-1$.
% and that the associated eigenvalues natural numbers.
...
Then, by continuity of~$\e^{t \mathcal L_{\rm FD}}$ on~$L^2_0(\kappa)$ for fixed $t \geq 0$,
\begin{equation}
...
\end{corollary}
\begin{proof}
    % Let us denote by $f_n$ the partial sum
    % \[
    %     f_n = \sum_{0 \leq \abs{\vect \alpha} \leq n} \ip{f}{\mathscr H_{\vect \alpha}} \mathscr H_{\vect \alpha}.
    % \]
    ...
    % because $L^2(\kappa)$ convergence implies almost everywhere convergence of a subsequence.
...
\begin{proof}
    ...
    %     &\leq  \e^{-\gamma t} \norm{f}
    %     + C \bigl( \norm{f} + \norm{\partial_q f} \bigr)
    %     \left( \frac{1}{\gamma} \e^{-\frac{\lambda_{\rm Lang}}{\gamma} t} + \frac{\e^{- \frac{\widetilde \ell}{\gamma}t} - \e^{- \gamma t}}{\gamma - \frac{\widetilde \ell}{\gamma}} \right) \qquad \forall \gamma \geq 1,
    % \end{align*}
    % which allows to conclude.
    % \[
    %     \ip{\e^{t \mathcal L} f}{f} \leq \norm{\e^{t \mathcal L} f} \norm{f}
    % \]
\end{proof}
...
% Consider
% \[
%     X_n = \left( 1 - \frac{1}{n} \right) \delta_0 + \frac{1}{n} \delta_{x_n}.
% \]
% where $x_n \to \infty$ in the limit as $n \to \infty$.
% Then
% \[
%     X_n \Rightarrow 0
% \]
% but unless
% \[
%     \frac{1}{n} x_n^{\alpha} \to 0,
% \]
% it does not hold that $\expect[X_n^{\alpha}] \to 0$.
