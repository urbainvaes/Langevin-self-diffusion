...
\begin{proposition}
    ...
    \begin{align}
        ...
        &\leq (d_{\phi} - d_{\psi})^2 + \gamma \beta^{-1} \norm{\grad_p \phi_{\vect e} - \grad_p \psi_{\vect e}}^2 (d_{\phi} + d_{\psi}) \\
        \notag
        &\leq 3 \gamma \beta^{-1} \norm{\grad_p \phi_{\vect e} - \grad_p \psi_{\vect e}}^2 (d_{\phi} + d_{\psi}).
    \end{align}
\end{proposition}
...
\begin{proof}
    ...
    \begin{align*}
        2 \norm{a}^4 + 2 \norm{b}^4 - 4\bigl(\ip{a}{b}^2\bigr)^2
        % &= 2 \norm{a}^4 + 2 \norm{b}^4 - \bigl(\ip{a}{a} + \ip{b}{b} - \ip{a-b}{a-b} \bigr)^2 \\
        &= \bigl(\norm{a}^2 - \norm{b}^2 \bigr)^2 + \norm{a-b}^2 \bigl(\norm{a}^2 + \norm{b}^2 \bigr) - \norm{a-b}^4 \\
        &\leq \bigl(\norm{a}^2 - \norm{b}^2 \bigr)^2 + \norm{a-b}^2 \bigl(\norm{a}^2 + \norm{b}^2 \bigr).
    \end{align*}
    Using this inequality with $a = \grad_p \phi_{\vect e}$ and $b = \grad_p \psi_{\vect e}$,
    we deduce
    \[
...
\begin{align*}
    ...
    &\quad + \frac{\Phi_N(q_{i+1}, p_{j+1}) - \Phi_N(q_{i+1}, p_{j}) - \Phi_N(q_{i}, p_{j+1}) + \Phi_N(q_{i}, p_{j})}{(q_{i+1} - q_i)(p_{j+1} - p_j)}  (q-q_i)(p-p_j),
\end{align*}
...
as the function $\psi_{\vect e}(q, t)$ in the definition of the control variate~\eqref{eq:definition_control_variate}.
...
\[
    d_{\psi_{\vect e}}
    = \int_{\torus^d \times \real^d} \abs{\grad_p \psi_{\vect e}}^2 \, \d \mu_{\delta}
...
where $\psi_1$ is constructed from an approximate solution to the Poisson equation in the one-dimensional setting,
...
Here $f_1 \otimes f_2$ denotes the function $(q,p) \mapsto f_1(q_1,p_1) \otimes f_2(q_2,p_2)$,
for two functions $f_1: \torus \times \real \rightarrow \real$ and $f_2: \torus \times \real \rightarrow \real$.
...
\begin{proposition}
    ...
    \begin{equation}
        ...
        \grad_p \phi_\delta \xrightarrow [\delta \to 0] {L^2(\mu_0)} \grad_p \phi_0.
    \end{equation}
\end{proposition}
\begin{proof}
    ...
    which gives
    \[
        \gamma \beta^{-1} \norm*{\grad_p \phi_\delta - \grad_p \phi_0^{\delta}}[L^2(\mu_{\delta})]^2
    ...
    \begin{equation}
        ...
        \gamma \beta^{-1} \norm*{\grad_p \phi_\delta - \grad_p \phi_0^{\delta}}[L^2(\mu_{\delta})]
        \leq C \delta \norm*{\grad_p \phi_0^{\delta}}[L^2(\mu_{\delta})],
    \end{equation}
...
\begin{remark}
    ...
    we deduce that,
    assuming the exact solution to the Poisson equation in one spatial dimension is employed in the control variate,
    the asymptotic variance is bounded from above by $C \delta^2 \gamma^{-4}$,
    which suggests an asymptotic scaling as $\delta/\gamma$ for the asymptotic relative standard deviation
...
they lead to estimators with reduced variance provided that the non-separable part of the potential is small with respect to \textcolor{red}{$\gamma$}.
...
\textcolor{red}{We now use this only for $\mathcal L_{\rm ovd}^{W} = \mathcal L_{\rm FD}$,
