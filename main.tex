...
\begin{lemma}
    ...
    \[
        \ip{\e^{t \mathcal L}p}{p} \leq C \gamma^{-2} + \e^{-\gamma t}
    \]
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    and the norm of $r(t)$ is exponentially decreasing in time as $C \e^{-\lambda t}$,
    for appropriate constants $C$ and $\lambda$ independent of~$\gamma$.
    ...
    \]
    Since there exist appropriate $C > 0$ and $\lambda > 0$ such that $\norm{\e^{t\gamma \mathcal L}}[\mathcal B\left(L^2_0(\mu)\right)] \leq C \e^{- \lambda t}$ for all $\gamma \in (1, \infty)$ ~\cite{roussel2018spectral,pavliotis2011applied},
    we deduce immediately that $\norm{u(t) - \widetilde u(t)} = \mathcal O(\gamma^{-1})$.
    The result then follows by noticing that $\norm{\widetilde u(t) - \widehat u(t)} = \mathcal O(\gamma^{-1})$ in the limit as $\gamma \to \infty$.
\end{proof}
...
\begin{remark}
    ...
    and it is possible to show for all $\lambda \in (0, 1)$ that there is $\varepsilon$ small enough such that
    \[
        \frac{1}{2}\derivative*{1}{t} |||u|||_N^2 \leq - 2 R(1 - \lambda) \, |||u|||_N^2.
    \]
    The reason for the suboptimal rate is that part of the term $\norm{\partial_q u}^2$, 
