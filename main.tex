...
explicit expressions of the bias or of velocity auto-correlation can be obtained:
...
\paragraph{Variance of the estimators.}%
We now obtain bounds on the variance of the estimators $u(T)$ and $v(T)$.
The two following results are of different nature:
the first one is non-asymptotic but
...
\begin{remark}
    ...
    This asymptotic behavior of the variance,
    which is in contrast with the usual behavior for estimators based on linear response,
    is made precise in \cref{proposition:asymptotic_variance} below.
    Consequently, there is an optimal choice of $T$ in order to minimize the mean-square error of estimators.
\end{remark}
\begin{remark}
    ...
    and they conjecture that also $\norm{\partial_p \phi}[\lp{p}{\mu}] = \mathcal O(\gamma^{-1})$.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= - \int_{0}^{T} \, (\vect e^\t p_t) \, \d t + \sqrt{2 \gamma \beta^{-1}} \int_{0}^{T} \, \grad_p \phi_{\vect e} (q_t, p_t) \cdot \d W_t, \\
        \psi_{\vect e}(q_T, p_T) - \psi_{\vect e}(q_0, p_0)
        &= \int_{0}^{T} \, \mathcal L \psi_{\vect e}(q_t, p_t) \, \d t + \sqrt{2 \gamma \beta^{-1}} \int_{0}^{T} \, \grad_p \psi_{\vect e} (q_t, p_t) \cdot \d W_t.
    \end{align*}
...
\begin{proof}
    ...
    The lower bound in~\eqref{eq:bounds_asymptotic_variance} follows from an application of Cauchy-Schwarz inequality,
    and the upper bound
    % Using that
    % \begin{align*}
    %      \gamma^2 \beta^{-2} \int \grad_p \phi_{\vect e} \cdot \grad_p \psi_{\vect e} \, \d \mu
    %      \leq  \left( \gamma^1 \beta^{-1} \int \grad_p \phi_{\vect e} \cdot \grad_p \phi_{\vect e} \, \d \mu \right)
    %     \left( \gamma^1 \beta^{-1} \int \grad_p \psi_{\vect e} \cdot \grad_p \psi_{\vect e} \, \d \mu \right)
    %     = d_{\psi} d_{\phi},
    % \end{align*}
    % we deduce
    % \begin{equation}
    %     \lim_{T \to \infty} \var \bigl(v(T)\bigr) \leq 2(d_{\phi} - d_{\psi})^2.
    % \end{equation}
