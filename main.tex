...
Yet another possible approach for estimating the mobility is to rely on a numerical approximation of the Green--Kubo formalism;
...
The following result result establishes that~$\phi_\delta \to \phi_0$ in $H^1(\mu_0)$ as~$\delta \to 0$.
...
\begin{proposition}
    ...
    \begin{equation}
        ...
        \norm*{\phi_\delta - \phi_0}[H^1(\mu_{0})]
        \leq C \delta \gamma^{-2}.
    \end{equation}
    % and denote
    % \[
...
\begin{proof}
    ...
    we deduce that there exists a constant $C_P > 0$ such that $\mu_{\delta}$ satisfies the Poincaré inequality uniformly:
    \begin{align}
    ...
    \end{align}
    Therefore, going back to~\eqref{eq:intermediate_delta},
    we have
    \begin{equation}
    ...
    and so $\norm*{\grad_{\vect p} \phi_0}[L^2(\mu_{\delta})] \leq C \norm*{\grad_{\vect p} \phi_0}[L^2(\mu_{0})]$.
    Since $\phi_0$ solves $- \mathcal L_0 \phi_0 = p_1$,
    we have
    \begin{align}
        \notag
        ...
        \leq \beta^{-1/2} \norm{\phi_0}[L^2(\mu_0)]
        \label{eq:bound_above_effective_diffusion}
    \end{align}
    Therefore,
    the right-hand side of~\eqref{eq:bound_gradient_of_difference} is bounded from above by $C \delta \gamma^{-2}$.
    (A bound like~\eqref{eq:bound_above_effective_diffusion}, but with a slightly better constant on the right-hand side,
    could in fact be obtained by employing the inequality~$\gamma D^{\gamma}_{\vect e} \leq D^{\rm ovd}_{\vect e}$ discussed in the introduction,
    but this is not necessary for our purposes.)

    In order to complete the proof~\eqref{eq:convergence_mobility_delta},
    it remains to show $\norm{\phi_{\delta} - \phi_0}[L^2(\mu_{0})] \leq C \delta \gamma^{-2}$.
    By the triangle inequality,
    it holds that
    \[
        \norm{\phi_{\delta} - \phi_0}[L^2(\mu_{\delta})]
        \leq
        \norm*{\phi_{\delta} - \phi^{\delta}_0}[L^2(\mu_{\delta})]
        + \norm*{\phi^{\delta}_0 - \phi_{0}}[L^2(\mu_{\delta})].
    \]
    The first term can be bounded by using~\eqref{eq:poincare_for_mudelta}.
...
Using~\eqref{eq:bound_gradient_of_difference},
we deduce that
\[
...
In particular, there exists $D_{} > 0$ such that
\[
    \forall (\gamma, \delta) \in (0, \infty) \times [-k, k], \qquad
    d[\phi_{\delta}] \geq 
\]

it holds that $\gamma d[\phi_{\delta}] \to 0$ in the limit as $\gamma \to 0$ provided that.

Combining the upper bound in~\cref{proposition:asymptotic_variance} with~\eqref{eq:bound_gradient_of_difference},
we deduce that,

if~$\psi_{\vect e} = \phi_0$ in the control variate~\eqref{eq:definition_control_variate},
then
\begin{align*}
    \lim_{T \to \infty} \var \bigl(v(T)\bigr)
    &\leq 4 \gamma \beta^{-1} \norm{\grad_{\vect p} \phi_{\vect e} - \grad_{\vect p} \psi_{\vect e}}^2 (d[\phi_{\vect e}] + d[\psi_{\vect e}])
    \leq C \delta^2 \gamma^{-4}
\end{align*}
for an appropriate constant $C$.
Given that $\expect \bigl( v(T) \bigr)$
