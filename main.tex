...
It is difficult to obtain bounds that scale well as $\gamma \to \infty$,
so we aim to obtain bounds with a good scaling in the underdamped regime only.
...
which enters in the definition~\eqref{eq:definition_control_variate} of the estimator~$v(T)$,
...
The parameters employed in the simulation are summarized in~\cref{table:parameters_employed_for_the_construction_of_the_control_variate},
and the effective diffusion coefficients calculated using the two approaches outlined,
together with the associated variances,
are illustrated in for a range of friction coefficients in \cref{fig:effective_diffusion_langevin}.
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/diffusion.pdf}
