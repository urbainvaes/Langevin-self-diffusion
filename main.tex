...
so we could simplify this result a bit.}
\begin{lemma}
    ...
    Let also $\mathcal L_{\rm ovd}^W = - \grad W(p) \cdot \grad + \beta^{-1} \Delta$ denote the generator of overdamped Langevin dynamics in potential~$W$.
...
\begin{proof}
    ...
    and noting that $\mathcal L_{\rm ovd}^W = - \grad^* \cdot \grad$,
    we obtain
    \[
        \frac{1}{2} \derivative*{1}{t} \seminorm{u}_j^2 = - \norm*{\grad (-\mathcal L_{\rm ovd}^W)^{j/2} u}^2.
    \]
...
\begin{lemma}
    ...
    \[
        ...
        \norm*{\grad^i \e^{t \mathcal L_{\rm ovd}} f} \leq \frac{C_{ij}}{t^{\frac{i-j}{2}}} \norm*{f}[j]
    \]
\end{lemma}
\begin{proof}
    Let $u(t) = \e^{t \mathcal L_{\rm ovd}} f$.
    We will show the existence of constants $C_{ij}$ such that
    \[
        \forall t \in (0, 1], \qquad
        \ip{(-\mathcal L)^i u(t)}{u(t)}
        \leq \frac{C_i}{t^{\frac{i-j}2}} \ip{(-\mathcal L)^j f}{f},
    \]
    after which the statement follows easily from the expression $\mathcal L = - \grad^* \cdot \grad$ and using commutator relations.
...
we confine ourselves from now to the one-dimensional setting in the proofs,
