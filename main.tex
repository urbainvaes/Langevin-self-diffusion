...
\begin{align*}
    ...
    &\quad + \frac{\Phi_N(q_{i}, p_{j+1}) - \Phi_N(q_{i}, p_{j}) - \Phi_N(q_{i+1}, p_{j+1}) + \Phi_N(q_{i+1}, p_{j})}{(q_{j+1} - q_j)(p_{j+1} - p_j)}  (q-q_i)(p-p_j).
\end{align*}
The domain size $L_p$ in the $p$ direction is chosen in such a way that escaping the domain is extremely unlikely during the simulation.
The coefficient $D_\psi$ is calculated based on this interpolant,
by sampling 
