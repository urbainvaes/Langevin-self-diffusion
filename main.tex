...
\Cref{fig:effective_diffusion_time_gle} illustrates the evolution of $\expect \bigl[ u(t) \bigr]$ and $\expect \bigl[ v(t) \bigr]$ with respect to time,
...
The probability measure $\pi$ implicit in this notation is either obvious in the context or explicitly specified.
...
We also recall that $\mathcal L_{\rm FD} = - \beta^{-1} \grad_{\vect p}^* \cdot \grad_{\vect p}$ by integration by parts,
...
the right-hand side of this equation defines an inner product on~$H^1(\kappa) \cap L^2_0(\kappa)$,
...
which implies that the resolvent operator $(-\mathcal L_{\rm FD})^{-1}$ is bounded from~$L^2_0(\kappa)$ to $H^1(\kappa) \cap L^2_0(\kappa)$.
Since the embedding $H^1_0(\kappa) \subset L^2_0(\kappa)$ is compact~\cite[Section 8.5]{lorenzi2006analytical},
...
\end{equation}
with the series on the right hand side being convergent in $L^2(\kappa)$ and $H^1(\kappa)$.
The series on the right-hand side converges, in fact, also in $H^2(\kappa)$.
To see this, note that for $n > m$,
\begin{align*}
    ...
    &\leq \sum_{k=m+1}^{n} \sum_{\ell=m+1}^{n} \phi_{k} \phi_{\ell} \ip{\mathcal L_{\rm FD} e_{k}}{\mathcal L_{\rm FD} e_{\ell}}
    = \sum_{k=m+1}^{n} \lvert \ip{f}{e_k} \rvert^2,
\end{align*}
and so $\hess \phi_n$ is a Cauchy sequence in $L^2(\kappa)$.
Here $W(\vect p) = \frac{\vect p^2}{2}$ and we used the commutator relation $\commut{\partial_i}{\partial_j^*} = \beta  \partial_{ij} W$.
In fact, the convergence in $H^2(\kappa)$ of the right-hand side of~\eqref{eq:series_phi} follows from the fact that,
for any $u \in H^2(\kappa)$,
\[
    \norm{\grad u} = \sum_{i=1}^{\infty} \lambda_i \lvert \ip{u}{e_i} \rvert^2, \qquad
    \norm{\hess u} = \sum_{i=1}^{\infty} \lambda_i^2 \lvert \ip{u}{e_i} \rvert^2.
\]

From the foregoing calculation,
it is not difficult to show that $(-\mathcal L_{\rm FD})^{-1}$ is a bounded operator from $L^2_0(\mu)$ to $H^2(\mu) \cap L^2_0(\mu)$
and that, relatedly, the unbounded operator
\[
    \mathcal L_{\rm FD}: H^2(\kappa) \cap L^2_0(\kappa) \subset L^2_0(\kappa) \to L^2_0(\kappa)
\]
is closed.
% Generalizing the above derivation,
It is also simple to show the resolvent bound
\[
    \norm{(\lambda - \mathcal L_{\rm FD})^{-1}}[\mathcal B\left(L^2_0(\mu)\right)] \leq \frac{1}{1+\lambda},
\]
and so the classical Hille--Yosida theorem implies that $\mathcal L_{\rm FD}$ generates a contraction semigroup~$(\e^{t \mathcal L_{\rm FD}})_{t \geq 0}$ on $L^2_0(\mu)$,
with the decay estimate $\norm{\e^{t \mathcal L_{\rm FD}} f} \leq \e^{-t}$.
By continuity of~$\e^{t \mathcal L_{\rm FD}}$ in~$L^2_0(\kappa)$ we have
\[
    \e^{t \mathcal L_{\rm FD}} f
    = \sum_{i=0}^{\infty} \e^{- \lambda_i t} \ip{f}{e_i} e_i.
\]
We will use two corollaries of this representation formula:
\begin{itemize}
    \item Decay of derivatives:
\end{itemize}
