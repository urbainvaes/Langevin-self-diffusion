...
\begin{proof}
    ...
    \[
        ...
        \leq 6^2 t \, \expect \int_{0}^{t} \abs{\derivative{1}[\phi]{p}(q_s, p_s) - \derivative{1}[\psi]{p}(q_s, p_s)}^4 \, \d s
        = 6^2 t^2 \int \abs{\derivative{1}[\phi]{p} - \derivative{1}[\psi]{p}}^4 \d \mu.
    \]
    Likewise, the second factor in~\eqref{eq:bound_variance_two_factors} can be bounded as
    \begin{align*}
        \expect \left( |q_t - q_0 - \xi_t|^4 \right)
    &\leq 3^3 \left( \expect \left( 2 \norm{\phi + \psi}[L^4(\mu)]^4 + 6^2 t \norm{\derivative{1}[\phi]{p} + \derivative{1}[\psi]{p}}[L^{4}(\mu)]^4 \right) \right),
    \end{align*}
    which concludes the proof.
\end{proof}

\begin{proof}
    Using It\^o's isometry we obtain that,
    For large times, it holds approximately that
    \[
        \frac{1}{\sqrt{2t}}
        \begin{pmatrix}
            q_t - q_0 \\
    ...
    it holds by the general formula for the higher-order moments of multivariate Gaussian distributions (Isserlis' theorem)
    that
    \begin{align*}
    ...
    \end{align*}
    For the Galerkin control variate, it holds by construction $\int \psi p \, \d \mu = \int \psi \mathcal \psi \, \d \mu = D_{\psi}$,
    and so the variance of $v(t)$ is approximately $D^2 - D_{\psi}^2$.
\end{proof}
