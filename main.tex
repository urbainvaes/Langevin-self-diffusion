...
\usepackage[style=trad-abbrv,doi=true,url=false,isbn=false,backend=biber]{biblatex}
...
Interestingly,
the mobility coincides
(up to the factor $\beta$)
with the so-called effective diffusion coefficient associated with the equilibrium dynamics~\eqref{eq:langevin},
which opens the door to a simple Monte Carlo approach for its estimation.
This link is made precise in the next paragraph, 
where we also define the effective diffusion coefficient.
The equation relating mobility and diffusion is called Einstein's relation,
and its derivation in simple settings dates back to Sutherland~\cite{sutherland1905lxxv}, Einstein~\cite{einstein1905molekularkinetischen} and Smoluchowski~\cite{von1906kinetischen} in the early 1900s.
For a rigorous justification of the relation in the specific setting of the Langevin dynamics~\eqref{eq:langevin},
...
It can be shown that a functional central limit theorem holds for the equilibrium Langevin dynamics~\eqref{eq:langevin},
...
to a Brownian motion in $\real^d$ with a matrix prefactor~$\sqrt{2 \mat D^{\gamma}}$ -- the effective diffusion tensor --
...
The first claim, on convergence of the rescaled position process to a Brownian motion,
may be obtained using the homogenization technique pioneered by Bhattacharya in~\cite{MR663900},
...
the latter approach is discussed in, for example,~\cite[Corollary 1]{roussel2018spectral}.
...
\end{equation}
where $(q_t^{(j)}, p_t^{(j)})_{t \geq 0}$, for $1 \leq j \leq J$,
...
showing that the variance increases linearly with the integration time.
While more general than approaches based on~\eqref{eq:einsteins_formula},
in that it applies to all transport coefficients,
...
are tailored specifically for Langevin dynamics and would have to be modified for other dynamics.
...
This is not a restrictive assumption in our setting,
...
appears only as a factor in the variance,
...
\begin{lemma}
    Independently of the choice of $\psi_{\vect e}$,
...
\begin{remark}
    ...
    to evaluate $\psi(q_t, p_t)$ at times 0 and $T$ and the gradient $\grad_{\vect p} \psi(\vect q_t, \vect p_t)$ along the full trajectory $\bigl((\vect q_t, \vect p_t)\bigr)_{0\leq t\leq T}$.
    It is therefore important for efficiency that $\grad_{\vect p} \psi$ is not computationally expensive to evaluate.
\end{remark}
...
It turns out that the estimate~\eqref{eq:bias} is in fact not optimal in the overdamped regime.
...
\begin{proposition}
    ...
    Then there exist constants $K$ and $\kappa$ independent of $f$ and $h$ such that
    \begin{equation}
        \label{eq:optimal_decay_correlation}
        \forall \gamma \geq 1, \quad
        \forall t \geq 0, \qquad
        \abs{\ip{\e^{t \mathcal L}f}{h}}
        \leq K \norm{f}[1,\vect q]  \norm{h}[1,\vect q] \left( \gamma^{-2} \e^{- \kappa \gamma^{-1} t} + \e^{-\kappa  \gamma t} \right),
    \end{equation}
...
\begin{example}
    ...
    \begin{align*}
        ...
        &= \frac{1}{2T} \Bigl( \gamma^{-2} \beta^{-1} (\e^{-\gamma T} - 1)^2
            + 2 \gamma^{-1} \beta^{-1} \left(T - 2 \gamma^{-1} (1 - \e^{-\gamma T}) + (2 \gamma)^{-1} (1 - \e^{-2 \gamma T})\right) \Bigr) \\
...
\begin{remark}
    ...
    and so it is not clear that employing a control variate is useful in this limit.
    Since our focus in this work is on the underdamped limit $\gamma \to 0$,
    and since the limit $\gamma \to \infty$ is more easily studied numerically through deterministic methods anyway,
...
\begin{proof}
    ...
    and using the short-hand notation $\chi_{s} = \chi_{\vect e}(\vect q_s, \vect p_s)$ for $\chi \in \{\phi_{\vect e}, \psi_{\vect e}\}$,
    we bound
    \begin{align*}
...
\begin{proof}
    ...
    \[
        ...
        \right),
    \]
    where the domain of integration of all the integrals in th covariance matrix is $\torus^d \times \real^d$.
    ...
    \begin{align*}
        ...
        &= \bigl(\norm{a}^2 - \norm{b}^2 \bigr)^2 + \norm{a-b}^2 \norm{a + b}^2.
    \end{align*}
    Using this identity with $a = \grad_{\vect p} \phi_{\vect e}$ and $b = \grad_{\vect p} \psi_{\vect e}$ in the Hilbert space $L^2(\mu)$,
    we deduce that
    \begin{align*}
