...
\begin{proof}
    ...
    (This bound could in fact be refined by employing the inequality~$\gamma D^{\gamma}_{\vect e} \leq D^{\rm ovd}_{\vect e}$ discussed in the introduction,
...
Combining the upper bound in~\cref{proposition:asymptotic_variance} with~\eqref{eq:bound_gradient_of_difference} and~\eqref{eq:bound_above_effective_diffusion},
we deduce that, if~$\psi_{\vect e} = \phi_0$ in the control variate~\eqref{eq:definition_control_variate},
then
\begin{align*}
    \lim_{T \to \infty} \var \bigl(v(T)\bigr)
    &\leq 4 \gamma \beta^{-1} \norm{\grad_{\vect p} \phi_{\vect e} - \grad_{\vect p} \psi_{\vect e}}^2 (d[\phi_{\vect e}] + d[\psi_{\vect e}])
