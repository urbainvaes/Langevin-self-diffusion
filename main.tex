...
\begin{align*}
    ...
    - \ip{\e^{s \mathcal L} \Pi_p \mathcal L \psi}{\mathcal L \psi}.
\end{align*}
The last two terms are bounded using the general inequality~\eqref{eq:decay_semigroup_general}:
\[
...
\begin{align*}
    ...
    &\leq C \norm{p + (\id - \Pi_p) \mathcal L \psi} \left( \gamma^{-2} \e^{- \frac{\lambda}{\gamma}s } + \e^{- \lambda \gamma s} \right) \\
...
\begin{lemma}
    ...
    Assume that $f \in L^2(\mu)$ is a smooth function such that
    \begin{equation}
...
\begin{proof}
    ...
    The space of functions of this form is dense in $({\rm Id} - \Pi_p) H^2(\mu)$,
...
\begin{proof}
    ...
    We show the result for functions of the form
    \begin{equation}
        \label{eq:expansion_2}
        f(q, p) = \sum_{i=0}^{N} \sum_{j=1}^{N} c_{ij} \, G_i(q) H_j(p),
    \end{equation}
    with the same notations as in the proof of \cref{lemma:initial_lemma}.
    The result then follows by density of functions of this type in $H^5(\mu)$.
    % Then by density,
    % there exists for any function $f \in H^5(\mu)$ a sequence of functions $\{f_n\}$ of the form~\eqref{eq:expansion}
    % converging to $f$ in $H^5(\mu)$, and so
    % \begin{align*}
    %     \abs{\ip{\e^{t \mathcal L}f}{h}}
    %     &= \lim_{n \to \infty} \abs{\ip{\e^{t \mathcal L}f_n}{h}} \\
    %     &\leq C \, \lim_{n\to \infty} \norm{f_n}_5  \norm{h}  \left( \gamma^{-2} \e^{- \lambda \gamma^{-1} t} + \e^{-\lambda  \gamma t} \right)
    %     = C \, \norm{f}_5  \norm{h}  \left( \gamma^{-2} \e^{- \lambda \gamma^{-1} t} + \e^{-\lambda  \gamma t} \right)
    % \end{align*}
    ...
    \begin{align}
        ...
        \Bigl( \mathcal L_{\rm Ham} \bigl(\e^{s \gamma \mathcal L_{\rm FD}} f\bigr) \Bigr)
         ...
         + ({\rm Id} - \Pi_p) \Bigl( \mathcal L_{\rm Ham} \bigl(\e^{s \gamma \mathcal L_{\rm FD}} f\bigr) \Bigr),
    \end{align}
    ...
    \begin{align}
        ...
        &\quad + \int_{0}^t \ip{\e^{(t-s) \mathcal L} ({\rm Id} - \Pi_p) \Bigl( \mathcal L_{\rm Ham} \bigl(\e^{s \gamma \mathcal L_{\rm FD}} f\bigr) \Bigr)}{f} \, \d s.
    \end{align}
    ...
    Given the assumption~\eqref{eq:expansion_2},
    ...
    \begin{align*}
        ...
        \norm{\e^{u \mathcal L} ({\rm Id} - \Pi_p) \Bigl( \mathcal L_{\rm Ham} \bigl(\e^{s \gamma \mathcal L_{\rm FD}} f\bigr) \Bigr)}
