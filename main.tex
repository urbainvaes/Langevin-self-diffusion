...
% \end{remark}
It is well known that the eigenfunctions of $\mathcal L_{\rm FD}$ are rescaled Hermite polynomials,
...
we have that, for any $f \in L^2_0(\mu)$,
\begin{equation}
...
\begin{proof}
    ...
    \end{equation}
    The first claim then follows from the inequality
    \(
    ...
    If $f \in L^2(\mu)$ does not belong to $D(\mathcal L_{\rm FD})$,
    then~\eqref{eq:gradient_series} may be employed to show that the series~\eqref{eq:series_expansion} is Cauchy and therefore convergent in~$H^1(\kappa)$ for fixed~$t$.
    Applying~\eqref{lemma:elliptic_reg} and~\eqref{lemma:overdamped_langevin_decay_derivatives} to the partial sum
    \[
        f_n = \sum_{0 \leq \abs{\vect \alpha} \leq n} \ip{f}{H_{\vect \alpha}} H_{\vect \alpha},
    \]
    which belongs to~$D(\mathcal L_{\rm FD})$,
    and then letting $n \to \infty$,
    we conclude that these inequalities are valid for the function spaces stated.
\end{proof}
