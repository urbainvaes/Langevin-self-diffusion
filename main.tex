...
this estimator has better properties than~\eqref{eq:simple_estimator},
at least in certain parameter regimes,
in term of bias and variance, respectively.

\subsection{Construction of an improved estimator}%
\label{sub:construction_of_an_improved_estimator}
In order to motivate the construction of an improved estimator,
...
The estimator~\eqref{eq:simple_estimator_improvement_1} can be further modified by replacing the expectation \eqref{eq:simple_estimator_improvement_1} by its value in the limit as $T \to \infty$;
that is, we define
\begin{equation}
...
\end{equation}
The expectation of $v(T)$ is now different from that of $u(T)$,
but the two expectations coincide asymptotically as $T \to \infty$.
Furthermore, the limit in~\eqref{eq:improved_estimator} can be calculated explicitly,
and, unlike $u(T)$,
the estimator $v(T)$ is unbiased if $\psi_{\vect e} = \phi_{\vect e}$ even for finite $T$.
\begin{lemma}
    It holds that
    \[
...
but we expect $\widehat u(T)$ to have lower variance.

In the usual control variate approach,
one adds and subtracts a random variable of which we know the expectation


We propose the following estimator of $D^{\gamma}_{\vect e}$ as an improvement over~\eqref{eq:simple_estimator}:
\begin{subequations}
\begin{equation}
    \label{eq:improved_estimator}
...
\end{align}
\end{subequations}
provided that $\psi_{\vect e}$ is a sufficiently good approximation of $\phi_{\vect e}$:
