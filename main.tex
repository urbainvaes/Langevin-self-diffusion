...
Since we are concerned only with one-dimensional dynamics in this section,
...
If $\psi_{\vect e} = \Psi_N$ and its gradient are stored as arrays of Fourier/Hermite coefficients,
this is computationally expensive,
because an evaluation of either function at a configuration $(q, p)$ requires to evaluate all the basis functions~\eqref{eq:basis_functions} at this configuration.
...
the computational cost of evaluating~$\widehat \Psi_N$ at a point is independent of $N_q$ and $N_p$.
...
which is essentially a simpler version of more general results such as~\cite[Theorem 2.4]{ottobre2012asymptotic} and~\cite[Theorem A.8]{MR2562709} in the hypoelliptic setting.
\begin{lemma}
...
when $\gamma \gg 1$,
a small prefactor $\gamma^{-1}$ multiplies the slow exponential $\e^{-\frac{\lambda t}{\gamma}}$,
...
\begin{proof}
    ...
    Applying~\cref{lemma:elliptic_reg} with the operator $\mathcal L_{\rm ovd} = \gamma \mathcal L_{\rm FD}$ over the time interval $[0, \gamma^{-1}]$,
    then \cref{lemma:overdamped_langevin_decay_derivatives} over $[\gamma^{-1}, t]$ (noting that $\e^{-\gamma (t - \gamma^{-1})} = C \e^{-\gamma t}$ for a constant $C$ independent of $\gamma$),
    and using the fact that $\kappa$ satisfies~\eqref{eq:poincare} with constant $R = \frac{\beta}{2}$,
    we obtain
    \begin{align}
    ...
    provided that the prefactor on the right-hand side in that equation is sufficiently large.
    ...
    Defining $\widetilde \lambda_{\rm Lang}$ in this manner ensures that the integral on the last line is bounded.
