...
Since the simple setting we consider enables a short and simple proof,
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \lVert (\lambda - \mathcal L_{\rm FD})^{-1} \rVert_{\mathcal B\bigl(L^2_0(\kappa)\bigr)} \leq \frac{1}{1 + \lambda}.
    \end{equation}
    ...
    and therefore also from~$L^2_0(\kappa)$ to $L^2_0(\kappa)$,
    ...
    the operator~$(-\mathcal L_{\rm FD})^{-1}$ is bounded from $L^2_0(\mu)$ to $H^2(\mu) \cap L^2_0(\mu)$,
    and so the unbounded operator~\eqref{eq:unbounded_operator} is closed.
\end{proof}

\begin{remark}
    \label{remark:alternative_norm}
    The calculation in~\eqref{eq:calculation} can be generalized to show 
    that the following functional defines a norm on~$H^i(\kappa)$ equivalent to~$\norm{\dummy}_i$:
    \[
        |\!|\!| u |\!|\!|_i = \sum_{n=1}^{\infty} (1 + \lambda_n^i) \ip{u}{e_n}^2.
    \]
\end{remark}
By continuity of~$\e^{t \mathcal L_{\rm FD}}$ in~$L^2_0(\kappa)$ we have
\[
    \e^{t \mathcal L_{\rm FD}} f
    = \sum_{n=0}^{\infty} \e^{- \lambda_n t} \ip{f}{e_n} e_n.
\]

As a direct corollary of~\cref{remark:alternative_norm} and this formula,
...
\begin{proof}
    ...
    For the second term, using that~$\commut{\partial_p}{\partial_p^*} = \beta$ we have
    \begin{align*}
