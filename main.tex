...
We select a final time $T$ and estimate the expectations and standard deviation of the estimators $u(T)$ and $v(T)$,
defined respectively in~\eqref{eq:simple_estimator} and~\eqref{eq:improved_estimator},
from a number $J$ of trajectories.
...
This is given by $\ip{\phi}{p}$, where $\ip{\dummy}{\dummy}$ is the inner product of $L^2(\mu_{\rm GLE})$
and $\phi$ is the unique solution in $L^2_0(\mu_{\rm GLE})$ to the Poisson equation
\[
...
with a prefactor that can be calculated efficiently and is different from $D^{\rm und}$.
...
\]
Indeed, if $\abs{\sqrt{x} - 1} \leq \varepsilon$ for $x \geq 0$ then $\abs{x-1} = \abs{\sqrt{x} - 1} \abs{\sqrt{x} + 1} \leq \varepsilon (2 + \varepsilon)$.
...
\end{align*}
which shows that the relative standard deviation admits an upper bound scaling as $\lvert \delta \rvert / \gamma$
in the limit as $\lvert \delta \rvert/\gamma \to 0$,
provided that $\delta \in [-1, 1]$ and $\gamma \in (0, 1)$.

\Cref{fig:time_bias_variance_2d} depicts the behaviour of the mobility with respect to $\gamma$ for various values of $\delta$.
