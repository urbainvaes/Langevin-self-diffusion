...
that this result is not optimal and,
using a novel decay estimate for correlation functions of the form $t \mapsto \ip{\e^{t \mathcal L} f}{h}$ when $f$ and $h$ are functions depending only on $p$,
we prove a finer bound on the bias in~\cref{proposition:better_bias}.

\begin{lemma}
    [Preliminary bound on the bias of the standard estimator]
    \label{lemma:easy_lemma}
    There exist constants~$M$ and~$\lambda$ such that
    \begin{equation}
        \label{eq:bias}
        ...
        \leq 
        % \frac{L}{\beta T}\int_{0}^{\infty} \exp\bigl(- \lambda \theta \min\{\gamma, \gamma^{-1}\}\bigr) \theta \, \d \theta =
        \frac{L \max \{\gamma^2, \gamma^{-2}\}}{\beta \lambda^2 T}.
    \end{equation}
\end{lemma}
...
\begin{equation}
    ...
    \norm*{ \e^{\theta \mathcal L} }[\mathcal B \left(L^2_0\left(\mu\right) \right)] \leq L \exp \bigl(- \lambda \theta \min\{\gamma, \gamma^{-1}\} \bigr)
\end{equation}
for appropriate constants $L > 0$ and $\lambda > 0$.
...
\begin{equation}
    ...
    \leq L \beta^{-1} \exp\bigl(- \lambda \theta \min\{\gamma, \gamma^{-1}\}\bigr),
\end{equation}
which leads to~\eqref{eq:bias}.
\end{proof}
...
which is clear in the case of quadratic potential, see~\eqref{eq:velocity_autocorrelation_quadratic}.
We derive a sharper estimate from the following result.
\begin{proposition}
    \label{proposition:semigroup_meanzero_observable}
    Let $\Pi_{\vect p}\colon \lp{2}{\mu} \rightarrow \lp{2}{\mu}; u \mapsto  u - \int u(\vect q, \vect p) \, \kappa(\d \vect p)$ and $\Pi_{\vect p}^\perp = \id - \Pi_{\vect p}$.
...
This result, the proof of which is postponed to \cref{sec:auxiliary_technical_results},
enables to prove the following bound on the bias of $u(T)$.
\begin{proposition}
    [Bias of the standard estimator]
    \label{proposition:better_bias}
    There exist constants~$K$ and~$\kappa$ such that
    \begin{equation}
        \label{eq:bias}
        \forall \gamma \in (0, \infty), 
        \quad
        ...
        \leq 
        % \frac{L}{\beta T}\int_{0}^{\infty} \exp\bigl(- \lambda \theta \min\{\gamma, \gamma^{-1}\}\bigr) \theta \, \d \theta =
        \frac{K \max \{1, \gamma^{-2}\}}{\beta \kappa^2 T}.
    \end{equation}
\end{proposition}
\begin{proof}
...
\begin{align*}
    ...
    &\leq L \e^{- \lambda \min\{\gamma, \gamma^{-1}\} t} \norm*{\vect e^\t \vect p + \mathcal L\psi_{\vect e}}  \left(\beta^{-1/2} + \norm{\mathcal L \psi_{\vect e}} \right),
\end{align*}
...
\[
    ...
    \leq L \exp \left( - \lambda \min\{\gamma, \gamma^{-1}\} t\right) \norm{\Pi_{\vect p} \mathcal L \psi_{\vect e}} \norm{\mathcal L \psi_{\vect e}},
\]
