...
\begin{equation}
    ...
    + \sqrt{2 \gnamma \beta^{-1}} \int_{0}^{T} \grad_{\vect p} \phi_{\vect e}(\vect q_t, \vect p_t) \cdot \d \vect w_t.
\end{equation}
...
\end{lemma}
\begin{proof}
    Given the definition of~\eqref{eq:improved_estimator},
    ...
    \begin{align*}
        ...
        = D^{\gamma}_{\vect e}.
    \end{align*}
    Here we used that $\mathcal L_{\rm ham}$ is antisymmetric in $L^2(\mu)$.
\end{proof}
...
\begin{remark}
    ...
    It is therefore important for efficiency that $\grad_{\vect p} \psi_{\vect e}$ is not computationally expensive to evaluate.
\end{remark}
...
\begin{remark}
    ...
    for example for the singular approximation~$\psi_{\vect e}$ constructed in~\cref{sub:underdamped_approach}.
    Although we leverage  we use approximations that are not twice differentiable in 
...
we obtain precise results on the bias and variance of the estimator $v(T)$.
We begin,
in \cref{sub:bias},
